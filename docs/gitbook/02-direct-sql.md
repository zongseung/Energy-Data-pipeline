...
psql·pandas·R 같은 도구로 PostgreSQL에 직접 연결해 조회하는 방법입니다. 재현 가능한 분석·통계·그래프 작업이라면 이쪽이 낫습니다.
...
3.  접속 상태를 확인합니다:
    ...
    목록에 DB 호스트가 보이면 연결된 상태입니다. 이 문서의 `<tailnet-host>`는 실제 호스트명으로 바꿔서 씁니다. 관리자가 안내하는 Tailscale MagicDNS 이름이나 tailnet IP입니다.
...
* 동시 연결 5개 제한
* **모든 쿼리가 로그로 남습니다** (`log_statement=all`) — 이용조건 참고
...
아래 쿼리는 전부 실제 DB에서 실행해 결과를 확인했습니다. 발전소명·기간만 바꿔 쓰면 됩니다. 단위·시간 규약·품질 등급은 데이터 카탈로그를 먼저 확인하세요.
...
{% hint style="warning" %}
**`data_quality`와 `is_aggregate`를 여기서 또 거를 필요는 없습니다.** `research.generation`은 시간별로 믿을 수 없는 구간을 **이미 제외한 상태**입니다. 여기에 `data_quality = '정상'`을 덧붙이면 영흥태양광 #3의 정상 구간(2025-07-01 이후)까지 날아가고 비태양광 46기(`미검증`)도 통째로 사라집니다. `is_aggregate = false`는 2019\~2021년 영암 발전량을 지웁니다 — 자세한 이유는 데이터 카탈로그의 is\_aggregate 절에 있습니다.
{% endhint %}

### 4-4. 발전량 × 일사량 결합

이 프로젝트는 발전소–기상관측소 최근접 매칭 테이블을 제공하지 않습니다 (좌표 신뢰도가 태양광도 ±2km 근사 수준이라 자동 매칭이 위험합니다 — 데이터 카탈로그 "좌표" 참고). 아래 예시는 `구미태양광`↔`구미` 관측소를 썼습니다. 발전소명과 관측소명이 우연히 겹치는 경우입니다. 실제 분석에서는 발전소 소재지를 직접 확인하고 `has_solar_sensor=true`인 관측소를 골라야 합니다:
...
`gen_kwh`(kWh) × `price`(원/kWh) = 원. `plants.region='mainland'`인 발전소는 `smp_hourly.region='land'`와 짝지어야 합니다(코드값이 다릅니다 — 데이터 카탈로그 참고).
...
`current_supply`/`supply_capacity` 컬럼명은 원천 항목과 뒤바뀌어 있습니다 — 공급능력이 필요하면 `current_supply`를 쓰세요 (데이터 카탈로그 참고).
