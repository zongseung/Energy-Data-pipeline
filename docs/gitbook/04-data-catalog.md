...
무엇이 얼마나 있고 컬럼이 무슨 뜻인지를 뷰마다 정리했습니다. 아래 목록에서
필요한 데이터를 고르면 해당 페이지로 이어집니다.

{% hint style="info" %}
**수치 조회 시점: 2026-08-13.** 아래 행수·기간은 이 시점에 DB에서 직접 조회한
값입니다. 데이터는 계속 쌓이므로 최신 값이 필요하면 각 절의 쿼리를 그대로 다시
실행하세요. 컬럼 의미는 DB의 `COMMENT`로도 확인할 수 있습니다.
...
<tbody>
<tr><td><strong>발전소와 발전량</strong></td><td>발전소 91기와 시간별 발전량. 함정이 가장 많습니다</td><td><a href="catalog/generation.md">catalog/generation.md</a></td></tr>
...
접속은 **pv DB 계정 하나로 끝납니다.** 수요·수급 5종은 물리적으로 다른 DB에
있지만 FDW로 연결돼 있어 발전량과 같은 쿼리에서 조인할 수 있습니다.
...
`timestamp` 값 `09:00`은 `[09:00, 10:00)` 구간을 뜻합니다. 원천(KPX·발전사)은
대부분 "구간이 끝난 시각"을 라벨로 쓰는 hour-ending 표기인데, 뷰가 이미
구간시작으로 옮겨서 내보냅니다. **직접 시프트하지 마세요.**
...
| demand·열수요 계열 | pv DB만큼 감사되지 않았습니다 |

### 등급 이름은 값 그대로 써야 합니다

`data_quality`는 한국어 값입니다. `정상`·`시간별무효`·`전면무효`·`미검증` 네
가지이며 영어로 번역하면 조회되지 않습니다.

특히 **`미검증`은 "품질이 나쁘다"가 아니라 "이 감사가 태양광만 봤다"는
뜻**입니다. 비태양광 46기가 여기 해당합니다 — 걸러낼 근거가 없다는 것과
정상이라는 것은 다릅니다.

### 필터를 덧붙이기 전에 뷰가 이미 거른 것을 확인하세요

`research.generation`은 시간별로 믿을 수 없는 구간을 **이미 잘라낸
상태**입니다. 여기에 `data_quality = '정상'`이나 `is_aggregate = false`를 더
얹으면 멀쩡한 데이터가 사라집니다. 이유는
[발전소와 발전량](catalog/generation.md) 페이지에 있습니다.

---

## 이 카탈로그를 읽는 순서

처음이라면 [발전소와 발전량](catalog/generation.md)부터 읽으세요. 함정이 가장
많고, 다른 페이지의 조인 대상이 대부분 이 두 뷰입니다.
