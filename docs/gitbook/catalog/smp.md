...
계통한계가격 3종. 하루전시장 시간별, 제주 실시간시장 15분, 가중평균입니다.
...
2010-01-01 이전은 육지·제주가 분리되지 않은 단일 시장이었습니다
...
{% hint style="warning" %}
**원천이 확정 공표를 멈췄습니다.** 2026-05-30 이후로 KPX가 확정 가격을 내지
않습니다. 수집기는 매일 정상 동작하지만 가져올 확정 데이터가 없습니다 — 우리
쪽 장애가 아닙니다.
{% endhint %}
...
우리 DB는 KPX가 확정한 마지막 날까지 이미 다 받아 뒀습니다. 원천이 재개되면
수집도 자동으로 이어집니다. 쓰기 전에 최신 시점을 확인하시려면:
...
존재**합니다. 최근 데이터나 제주 BLMP를 찾으면 결과가 빈 것이 정상입니다
(결측이 아니라 애초에 그 시기 이후 BLMP 자체가 발행되지 않습니다).
