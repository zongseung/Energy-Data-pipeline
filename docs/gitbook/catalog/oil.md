...
{% hint style="warning" %}
**현물 고시가가 아닙니다.** Hyperliquid XYZ builder DEX의 체결 캔들이라 원유
현물 종가와는 다른 계열입니다. 논문에 "브렌트유 가격"으로 인용하기 전에 이
출처가 목적에 맞는지 먼저 판단하세요.
{% endhint %}
...
이 뷰는 수집기가 쓰는 CSV 파일을 `file_fdw`로 **직접** 읽습니다. 중간 적재
단계가 없어서 수집이 끝나는 즉시 조회에 반영되고, 동기화 지연이 생길 여지가
없습니다.

원본 wide 형태가 필요하면 `research.oil_hourly_raw`를 보시면 됩니다(브렌트·
WTI가 한 행에 나란히 있는 형태).

#### 브렌트와 WTI의 시작일이 다릅니다

WTI는 2026-01-07부터, 브렌트는 2026-03-05부터입니다. 원천에 그만큼만
있어서이며 결측이 아닙니다. 두 종목을 함께 비교하는 분석은 겹치는 구간부터
잡으세요.
...
DEX라 주말·야간에도 캔들이 나옵니다. "일별 종가"를 만들 때 국내 장 마감 시각을
가정하지 마세요. 어느 시각을 종가로 볼지는 분석자가 정하셔야 합니다.
...
{% hint style="warning" %}
**`generation`을 집계하지 않고 그대로 3자 조인하면 90초를 넘깁니다.**
`research.generation`은 320만 행이라 발전소 × 시각만큼 행이 불어난 채로 조인에
들어갑니다. 위처럼 CTE에서 시각별로 먼저 접으면 168행짜리 조인이 되어
**0.7초**에 끝납니다. 같은 결과, 100배 이상 차이입니다.
{% endhint %}
