...
발전소 마스터(`research.plants`)와 시간별 발전량(`research.generation`). 이 프로젝트에서 함정이 가장 많은 두 뷰다 — 단위·시간 규약·품질 등급을 모르고 쓰면 답이 조용히 틀린다.
...
> **이 뷰는 시간별로 믿을 수 없는 구간을 이미 잘라낸 상태다.** `data_quality`가
> `전면무효`인 발전소 전체와, `시간별무효`인 발전소의 `hourly_valid_from` 이전
> 구간은 여기 없다. 그래서 `data_quality`로 한 번 더 거를 필요가 없다 — 오히려
> 이중으로 걸면 영흥태양광 #3의 2025-07-01 이후 정상 구간까지 사라진다.
>
> 왜 잘라냈나: 등급을 컬럼으로 보여주기만 했더니 아무도 필터를 걸지 않았고,
> 한밤중에 태양광이 발전한 것으로 나오는 답이 그대로 나갔다. 야간(21~04시)
> 태양광 9,981,569 kWh 중 **99.98%가 위 두 조건에 걸리는 행**이었다. 지금은
> 같은 구간이 1,623 kWh 로 줄었다(잔여는 정상 등급 발전소의 계량 노이즈).
>
> **발전소 개수를 세는 질문은 `research.plants`를 봐야 한다** — 이 뷰에는 시간별
> 신뢰 구간이 없는 발전소가 아예 등장하지 않기 때문이다. 잘려나간 구간도 *일별*
> 합계는 유효할 수 있으니(`daily_valid_from`~`daily_valid_to`), 필요하면 관리자에게
> 요청하라.
...
버린다.** 반드시 `hourly_valid_from`을 함께 확인하라.
{% endhint %}

영흥태양광 #3 계열 3기(`plant_id` 20/29/32)는 `시간별무효` 등급이지만
**2025-07-01부터 시간별 값도 정상**이다. 등급만 보고 거르면 2025-07부터
지금까지가 통째로 날아간다.
...
#### 함정 — is_aggregate (지금은 빼면 안 된다)

{% hint style="danger" %}
**`WHERE is_aggregate = false`를 넣지 마라.** 지금은 이중계상이 없고, 넣으면
2019~2021년 영암 발전량 5,947,954 kWh가 통째로 사라진다.
{% endhint %}

`plant_id 140`(`영암태양광`)은 `141`(영암1차)·`142`(영암2차)와 같은 사업의
**전체 통합 계열**이다. 원천이 2019~2021년 구간에 1차/2차를 구분해 주지 않아
통합값으로만 존재한다.
...
기간이 겹치지 않으므로 지금은 셋을 함께 합산해도 이중계상되지 않는다. 이
플래그는 "지금 중복"이 아니라 **"겹치면 중복될 계열"**이라는 표시다.
2019~2021 구간에 141·142가 백필되는 순간부터는 반드시 제외해야 한다.

<details>

<summary>이름을 바꾼 이유</summary>

이 행의 이름은 원래 `영암태양광_합계`였다. "합계"라는 말 때문에 "다른 행과
중복이니 빼야 할 줄"로 읽혀 실제로 오판이 발생해 사업명 그대로 바꿨다.
