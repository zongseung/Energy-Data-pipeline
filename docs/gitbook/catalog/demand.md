...
전국·제주 계통 수급과 지역난방 열수요. 물리적으로는 demand DB에 있지만 FDW로 연결돼 있어 pv DB 계정 하나로 발전량과 함께 조회합니다.

---

## 수요·수급 데이터 (demand DB, FDW 경유)

**따로 접속하실 필요 없습니다.** 아래 5개는 물리적으로 demand DB에 있지만 pv
DB의 `research` 스키마로 FDW 연결돼 있어 **pv DB 계정 하나로 발전량과 함께
조회하고 조인까지 할 수 있습니다.** 복제본이 아니라 라이브 프록시라 값도 항상
최신입니다.
...
> **FDW 조인 주의 — 기간 조건을 양쪽에 거세요.** 위 쿼리에서 `d.timestamp`
> 조건을 빼면 PostgreSQL이 원격 테이블 132만 행을 통째로 끌어와 2분이 지나도
> 안 끝납니다. 조인 조건만으로는 범위가 원격까지 전달되지 않습니다. 로컬 뷰와
> FDW 테이블을 조인할 때는 **양쪽 테이블 각각에** WHERE 로 기간을 명시하는
> 것이 규칙입니다.

이 계열의 시간 규약은 pv DB만큼 감사되지 않았습니다. **근거가 없는 컬럼은
시프트하지 않았습니다** — 아래 "무보정" 표기는 게으름이 아니라 확정 근거가
생기기 전까지 임의로 손대지 않겠다고 일부러 내린 결정입니다.
...
{% hint style="success" %}
`research` 스키마에서 가장 최신인 계열입니다. 10분마다 적재돼 조회 시점 기준
20분 전 데이터까지 들어와 있습니다.
{% endhint %}
...
> **끊긴 게 아니라 이게 전량입니다.** 19개 지사 모두 2021-01-01 ~ 2023-12-31
> 구간에 정확히 26,279행씩 들어 있습니다. 수집이 중간에 멈춰 잘린 게 아니라
> 처음부터 이 3년치가 데이터셋 전체입니다. 최신 데이터를 기다리지 마시고 그대로
> 쓰시면 됩니다. (다른 뷰와 달리 이 저장소에 수집 코드가 없습니다 — 외부에서
> 한 번 유입된 자료입니다.)
...
(88행, 2026-01-09~2026-05-31)**입니다. 지점별 분석에서는
`WHERE station_name <> 'UNKNOWN'`으로 제외하세요. 지점 수만큼 수요값이
반복되므로(지점 95~96개 × 같은 시각) 수요값만 필요하면 이 뷰 대신
`research.demand_5min`을 쓰는 것이 낫습니다.
