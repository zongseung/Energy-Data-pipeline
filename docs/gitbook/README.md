...
두 방법 모두 같은 Tailscale 폐쇄망과 같은 개인 계정을 사용합니다. 어느 쪽을 선택해도 보안 경계는 달라지지 않습니다. 전체 흐름은 데이터 제공 구조에서 확인하세요.

## 다음으로 읽을 페이지

* 데이터 제공 구조 — 두 조회 방법이 공유하는 보안 경계
* 직접 SQL로 조회 — Tailscale 연결부터 첫 쿼리까지
* LLM·MCP로 조회 — 로컬 MCP 설치부터 자연어 질문까지
