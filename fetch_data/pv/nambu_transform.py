...
def preprocess_nambu():
    ...
    if not raw_files:
        logger.warning("[!] 전처리할 원본 파일이 없습니다.")
        return

    logger.info(f"--- 총 {len(raw_files)}개 파일 전처리 및 시간축 전환 시작 ---")
    ...
    for file_path in raw_files:
        ...
        # 라. Timestamp 생성 및 정렬
        # 'qhorgen14' -> 14 (정수) 추출 (1-based hour: qhorgen01=1시, qhorgen24=24시)
        ...
        logger.info(f"  [OK] {file_path.name} -> {out_name} (변환 및 병합 완료)")
